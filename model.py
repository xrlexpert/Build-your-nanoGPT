...
def generate_text_simple(model, idx, max_new_tokens, context_size):
    ...
    for _ in range(max_new_tokens):
        ...
        topk_values, topk_indices = torch.topk(logits, k=5, dim=-1)  # (batch, 5)

        # Convert logits to probabilities using softmax
        probs = F.softmax(topk_values, dim=-1)  # (batch, 5)

        # Sample from the top-5 indices based on their probabilities
        idx_next = torch.multinomial(probs, num_samples=1)  # (batch, 1)
        idx_next = topk_indices.gather(-1, idx_next)  # (batch, 1)
...
def generate_and_print_sample(model, tokenizer, device, context_size, start_context,):
    model.eval()
